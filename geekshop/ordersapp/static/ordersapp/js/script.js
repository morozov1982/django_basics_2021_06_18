window.onload = function () {
    ...
    function orderSummaryUpdate(orderitem_price, delta_quantity) {
        ...
        $('.order_total_quantity').html(orderitem_quantity.toString());
    }

    function deleteOrderItem(row) {
        const target_name = row[0].$('input[type="number"]').name;  // было: querySelector('input[type="number"]').name;
    ...
    $('.order_form select').change(function (event) {
        const target = event.target;
        orderitem_num = parseInt(target.name.replace('orderitems-', '').replace('-quantity', ''));
        const orderitem_product_pk = target.options[target.selectedIndex].value;
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.price) {
                        ...
                        const price_html = '<span>' + data.price.toString().replace('.', ',') + '</span> руб.';
                        const current_tr = $('.order_form table').find('tr:eq(' + (orderitem_num + 1) + ')');
